...
int main() {
    ...
    for (int i = 0; i < BUFFER_SIZE; i++) {
        tx_buf[i] = i;  // input data: write to cached memory
    }

    // Send from DDR -> DMA MM2S -> bram_stream_writer
    xil_printf("Starting MM2S DMA to bram_stream_writer...\r\n");
    Xil_DCacheFlushRange((UINTPTR)tx_buf, BUFFER_SIZE * sizeof(u32)); // flush cached input
    ...
    // Read from bram_stream_reader -> DMA S2MM -> DDR
    xil_printf("Starting S2MM DMA from bram_stream_reader...\r\n");
    Xil_DCacheInvalidateRange((UINTPTR)rx_buf, BUFFER_SIZE * sizeof(u32)); // invalidate cache when reading
