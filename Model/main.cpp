...
//#define TRMULTPATH "C:/Research/CU-Spur-Sean/C/input/trmult_reduced.bin"
#define TRMULTPATH "/mnt/c/Users/mrflu/OneDrive/Desktop/School/research/FPGA_acceleration/drewIOFiles/input/trmult_reduced.bin"
...
//#define INPUTPATH "C:/Research/CU-Spur-Sean/C/input/"
#define INPUTPATH "/mnt/c/Users/mrflu/OneDrive/Desktop/School/research/FPGA_acceleration/drewIOFiles/input/"
//#define PRINTPATH "C:/Research/CU-Spur-Sean/C/output/"
#define PRINTPATH "/mnt/c/Users/mrflu/OneDrive/Desktop/School/research/FPGA_acceleration/drewIOFiles/output/"
#ifdef DEBUG_MODE
  //#define DEBUGPATH "C:/Research/CU-Spur-Sean/C/input/debug/"
  #define DEBUGPATH "/mnt/c/Users/mrflu/OneDrive/Desktop/School/research/FPGA_acceleration/drewIOFiles/input/debug/"
#endif
#define N_MODEL 1
...
// ==================================== END NLLS
void loop_uhat_sparse(env_t *env, real tol,real *L, real L_exp,
...
void loop_uhat_eigen(const real tol,const real *trmult_reduced,const real *L,const real L_exp,
 ...
 */
{
    ...
    real error = TOLL_MODEL_INITIAL_PERIOD + 1.; // Initialize error
    int iter = 0;
    real *trmult_row = (real*) malloc(NPOSLAND * sizeof(real)); // Buffer for one row
    real *row_error = (real*) malloc(sizeof(real));

    assert(trmult_row!=NULL);
    assert(row_error!=NULL);

    printf("Starting loop_uhat\n");

    while (error >= TOLL_MODEL_INITIAL_PERIOD) 
    {
        error = 0; // Reset error for this iteration

        for (int i = 0; i < NPOSLAND; i++) 
        {

            // Load one row into buffer from trumult_reduced
            memcpy(trmult_row, &env->trmult_reduced[IPOSLAND(i,0)], NPOSLAND * sizeof(real));

            // Call FPGA function for one row (integral computed inside FPGA)
            loop_uhat_row(i, TOLL_MODEL_INITIAL_PERIOD, trmult_row, FL, env->uhat_f_exp, uhat_i, FR, env->exp_uhatR, row_error);

            // Update total error from FPGA output
            error += *row_error;
 ...
 */
{
    ...
    real error = TOLL_M2_INNER + 1.; // Initialize error
    int iter = 0;
    real *trmult_row = (real*) malloc(NPOSLAND * sizeof(real)); // Buffer for one row
    real row_error = 0; // Stores error for each row
    while (error >= TOLL_M2_INNER) 
    {
        error = 0; // Reset error for this iteration
        ...
        {
            // Load one row into buffer from trumult_reduced
            memcpy(trmult_row, &env->trmult_reduced[i * NPOSLAND], NPOSLAND * sizeof(real));

            // Call FPGA function for one row (integral computed inside FPGA)
            loop_uhat_row(i, TOLL_M2_INNER, trmult_row, ML, L_exp, m2_i, MR, R_exp, &row_error);
        ...
        iter++;
    #ifdef DEBUG_MODE_ERROR_UHAT
        printf("iter %d error_m2: %.16lf\n", iter, error);
    #endif
    }
    free(trmult_row);
    #ifdef DEBUG_MODE_ERROR_UHAT
    printf("Final error_m2: %.16lf\n", error);
    #endif
    ...
    for (int t = 0; t < T_FORWARD; t++)
    {
        ...
        while (error_realgdp >= FW_tol_realgdp)
        {
            ...
            while (error_e >= FW_tol_e)
            {
                ...
                real error = BW_tol + 1.; // Initialize error
                int iter = 0;
                real *trmult_row = (real*) malloc(NPOSLAND * sizeof(real)); // Buffer for one row
                real row_error = 0; // Stores error for each row
                while (error >= BW_tol) 
                {
                    ...
                    for (int i = 0; i < NPOSLAND; i++) 
                    {
                        ...
                        // Call FPGA function for one row (integral computed inside FPGA)
                        loop_uhat_row(i, BW_tol, trmult_row, FL, env->uhat_f_exp, uhat_i, FR, env->exp_uhatR, &row_error);
            ...
            while (error_e >= BW_tol_e)
            {
                ...
                while (error >= BW_tol) 
                {
                    ...
                    for (int i = 0; i < NPOSLAND; i++) 
                    {
                        ...
                        // Call FPGA function for one row (integral computed inside FPGA)
                        loop_uhat_row(i, BW_tol, trmult_row, BL, env->uhat_f_exp, uhat_i, BR, env->exp_uhatR, &row_error);
