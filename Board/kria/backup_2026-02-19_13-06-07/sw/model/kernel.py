...
class MVMKernel:
    def __init__(
        ...
        element_width_bits: int = 16, # 16/32/64
        ...
        self.partition_base = [
            self.bram_base + i * self.bytes_per_partition
    ...
    def send_vector(self, vec_partitions: np.ndarray):
        """
        vec_partitions: shape (NUM_PARTITIONS, ELEMENTS_PER_PARTITION), dtype self.np_dtype
        Copies into CMA and uses CDMA to write to BRAM partitions.
        """
    ...
    def matvec(self, matrix_row_source=None):
        """
        Runs one matrix-vector multiply over the *already programmed* vector in BRAM.
        The overlay is expected to:
          - take streamed matrix rows (DMA send)
          - produce one output per row (DMA recv)

        Returns:
          out: np.ndarray shape (NUM_ROWS_TOTAL,) float64
        """
        if self.verbose:
            print("[matvec-kernel] START")

        # receive buffers first
        for ch, d in enumerate(self.dmas):
            d.recvchannel.transfer(self.result[ch])

        # choose source (default to full matrix memmap)
        src = self.matrix_data if matrix_row_source is None else matrix_row_source
        ...
        if self.verbose:
            print("[matvec-kernel] FINISH")

        # flatten and remove padding
        out64 = self.result.reshape(-1)[:self.original_n]
        return out64.astype(np.float32, copy=True)
