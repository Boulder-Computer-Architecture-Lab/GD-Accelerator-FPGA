...
ITER = 3 # Send N elements this many times
...
if __name__ == "__main__":
    ...
    for i in range(ITER):
        ...
        dma0_proc = subprocess.Popen([
            "./dma_transfer", "mm2s",
            hex(a_buf.physical_address), str(N * 8), hex(DMA0_BASE) # N+1 temp fix
        ])
        ...
        dma1_proc = subprocess.Popen([
            "./dma_transfer", "mm2s",
            hex(BRAM_BASE + i*N*8), str(N * 8), hex(DMA1_BASE) # N+1 temp fix
        ])
    ...
    a_mat = np.vstack(a)  # Shape: (ITER, N)
    expected = sum(np.dot(a_mat[i], b[i*N:(i+1)*N]) for i in range(ITER))
