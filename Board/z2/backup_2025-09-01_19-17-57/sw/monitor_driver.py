...
class APM:
    ...
    def _read_live64(self, index):
        base = self.LIVE_BASE + index * self.LIVE_STRIDE
        lo = self.mmio.read(base) & 0xFFFFFFFF
        hi = self.mmio.read(base + 4) & 0xFFFFFFFF
        return (hi << 32) | lo
    ...
    def read_slot(self, which):
        ...
        """
        xfer      = self._read_live64(idx_base + 0)
        bytes_    = self._read_live64(idx_base + 1)
        slv_idle  = self._read_live64(idx_base + 2)
        mst_idle  = self._read_live64(idx_base + 3)

        total = xfer + slv_idle + mst_idle
        secs  = (total / self.clk_hz) if self.clk_hz else 0.0
        bw    = (bytes_ / secs) if secs > 0 else 0.0

        if bytes_ == 0: bytes_ = xfer * (64/8)
        seconds = total / self.clk_hz if total else 0.0
        bw = bytes_ / secs if secs else 0.0
